...
char *pick_addr() {
  size_t offset = (rand() << 12) %  (mem_size / 4);
  return g_mem + offset;
}
...
static void toggle(int iterations, int addr_count) {
  ...
  for (int j = 0; j < iterations; j++) {
    __m128i *addrs[addr_count];
    for (int a = 0; a < addr_count; a++)
      addrs[a] = (__m128i *) pick_addr();

    uint32_t sum = 0;
    for (int i = 0; i < toggles; i++) {
      for (int a = 0; a < addr_count; a++) {
          __m128i res = _mm_stream_load_si128(addrs[a]);
          uint32_t* res_32 = (uint32_t*) &res;
          for (int j = 0; j < 4; ++j) {
            sum += res_32[j] + 1;
          }
      }
